...
<body>
	...
	<div style="width: 100%; height: 1500px; border: none;">
		...
		<div id="menu_bar" style="height: 100%; width: 15%; float: left;">
			<div class="btn-group-vertical" role="group" aria-label="..."
				style="height: 100%; width: 95%;"></div>
		</div>

		<!-- 스크롤이 300이상시 빈공간을 채워준다.(row 3 - col 1-1) -->
		<div id="menu_bar_after" style="float: left;"></div>
		...
		<div style="height: 100%; float: left; width: 70%; overflow: auto;">
			...
			<table class="board_list">
				...
				<tbody>
					<tr>
						<td class="title"><a href='#this' name='back'> ... </a></td>
					</tr>
					...
					<c:choose>
						<c:when test="${fn:length(list) > 0}">
							<c:forEach items="${list}" var="row" varStatus="status">
								...
								<c:set var="originalPathLength"
									value="${fn:length(originalPath)}" />
								<c:set var="length" value="${fn:indexOf(originalPath, \"/\")}" />
								<c:set var="partialPath"
									value="${fn:substring(originalPath, currentPathLength, originalPathLength)}" />
								...
								<c:if test="${fn:startsWith(originalPath, currentPath)}">
									...
									<c:if test="${fn:contains(partialPath,\"/\" )}">
										<!-- 디렉토리의 경우 경로 해당 -->
										<c:set var="directoryLength"
											value="${fn:indexOf(partialPath, \"/\")}" />
										<c:set var="directory"
											value="${fn:substring(partialPath, 0,directoryLength)}" />
										...
										<c:if test="${(directory ne temp)||(temp eq init)}">
											...
											<tr>
												<td class="title"><span
													...
													<a href='#this' name='directory'> &nbsp ${directory} </a> <c:set
														var="temp" value="${directory}" /> <c:set var="init"
														value="1" /></td>
											</tr>
										</c:if>
									</c:if>
								...
								<c:if test="${fn:startsWith(originalPath, currentPath)}">
									<c:if test="${not fn:contains(partialPath,\"/\" )}">
										<tr>
											<td class="title"><span class="glyphicon glyphicon-file"
												aria-hidden="true"></span> <input type='hidden' id='IDX'
												value="${row}"> <a href='#this' name='title'>&nbsp${partialPath}</a>
	...
	<footer
		style="height: 50px; width: 100%; float: left; background-color: #B26B69;">
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("a[name='back']").on("click", function(e) { //제목 
				e.preventDefault();
		...
		function fn_openDirectory(obj) {
			...
			comSubmit.addParam("FILEPATH", PATH
					+ obj.parent().find("#DIR").val());
